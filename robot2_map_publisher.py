...
class Robot2MapRepublisher:
    def __init__(self):
        # Get current domain ID from environment variable
        current_domain_id = int(os.environ.get('ROS_DOMAIN_ID', '0'))
        
        # Set ROBOT2_TARGET_DOMAIN_ID environment variable for other nodes to use
        os.environ['ROBOT2_TARGET_DOMAIN_ID'] = str(current_domain_id)
        ...
        self.subscriber_node = Node('robot2_map_subscriber', context=self.subscriber_context)
        ...
        self.subscriber_node.get_logger().info(f"Set ROBOT2_TARGET_DOMAIN_ID environment variable to: {current_domain_id}")
        
        # Create publisher node on domain 0
        self.publisher_node = Node('robot2_map_publisher', context=self.publisher_context)
        ...
        # Parameters
        self.publisher_node.declare_parameter('map_frame', 'robot2/map')
        ...
        self.subscriber_node.get_logger().info(f"Robot2 map republisher initialized")
        self.subscriber_node.get_logger().info(f"Subscribing to /map on domain {current_domain_id}, publishing to /robot2/map on domain 0")
        self.subscriber_node.get_logger().info(f"Subscribing to /gps on domain {current_domain_id}, publishing to /robot2/gps_position on domain 0")
        self.subscriber_node.get_logger().info(f"Publishing robot2 position to /robot2/position on domain 0")
    ...
    def get_robot_position(self):
        ...
        try:
            ...
            # Convert to PoseStamped with robot2/map frame
            pose = PoseStamped()
            pose.header.frame_id = 'robot2/map'  # Use robot2's own frame
    ...
    def publish_position(self):
        """Publish robot2 position in robot2/map frame"""
        position = self.get_robot_position()
        if position is not None:
            self.position_pub.publish(position)
            self.subscriber_node.get_logger().debug(f"Published robot2 position in robot2/map frame: ({position.pose.position.x:.2f}, {position.pose.position.y:.2f})")
        else:
            self.subscriber_node.get_logger().debug("Could not get robot2 position for publishing")

    def map_callback(self, msg):
        """Callback to republish /map to /robot2/map with updated frame_id"""
        try:
            self.map_received_count += 1
            if self.map_received_count % 10 == 1:  # Log every 10th message
                self.subscriber_node.get_logger().info(f"Received map message #{self.map_received_count}, republishing to /robot2/map")
            ...
            # Publish to /robot2/map on domain 0
            self.map_pub.publish(republished_map)
            
            if self.map_received_count % 10 == 1:  # Log every 10th message
                self.subscriber_node.get_logger().info("Successfully republished map to /robot2/map on domain 0")
        except Exception as e:
            self.subscriber_node.get_logger().error(f"Error republishing map: {e}")
    
    def gps_callback(self, msg):
        """Callback to republish /gps to /robot2/gps_position"""
        try:
            self.gps_received_count += 1
            if self.gps_received_count % 5 == 1:  # Log every 5th GPS message
                self.subscriber_node.get_logger().info(f"Received GPS message #{self.gps_received_count}, republishing to /robot2/gps_position")
            ...
            # Publish to /robot2/gps_position on domain 0
            self.gps_pub.publish(republished_gps)
            
            if self.gps_received_count % 5 == 1:  # Log every 5th GPS message
                self.subscriber_node.get_logger().info(f"Successfully republished GPS to /robot2/gps_position on domain 0: "
...
def main(args=None):
    republisher = Robot2MapRepublisher()
    try:
        republisher.spin()
    except KeyboardInterrupt:
        pass
